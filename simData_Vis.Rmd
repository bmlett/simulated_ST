...
geneNamesKey = data.frame("KEY" = 1:dim(count)[1], "Value"= count$X)
...
```{r include=FALSE}
df1 = data.frame(matrix(0,nrow=length(count$X),ncol = length(comp$X)))
...
```{r include=FALSE}
df2 = data.frame(matrix(0,nrow=length(count$X),ncol = length(comp$X)))
for (i in 1:length(comp$X)) {
  t = comp[i,2:101]
  x = which(t!=0)
  print(comp$X[i]) 
...
```{r include=FALSE}
df3 = data.frame(matrix(0,nrow=length(count$X),ncol = length(comp$X)))
for (i in 1:length(comp$X)) {
  t = comp[i,2:101]
  x = which(t!=0)
  print(comp$X[i]) 
